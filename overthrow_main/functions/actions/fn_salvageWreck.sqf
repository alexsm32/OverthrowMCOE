...
private _doSalvage = {
  [_this] spawn {
	...
	format["desguazando vehiculo en %1",_toname] call OT_fnc_notifyMinor;
    ...
    for "_x" from 0 to (_steel - 1) do {
        if(!(_veh isKindOf "Truck_F" || _veh isKindOf "ReammoBox_F") && !(_veh canAdd "OT_Steel")) exitWith {
            "No tienes mas sitio, usa un camion o una caja" call OT_fnc_notifyMinor;
        };
        _done = _done + 1;
    ...
    if(_plastic > 0 && ((_veh isKindOf "Truck_F" || _veh isKindOf "ReammoBox_F") || (_veh canAdd "OT_Plastic"))) then {
        ...
        format["Desguazado: %1 x Acero, %1 x Plastico",_done,_plastic] call OT_fnc_notifyMinor;
    }else{
        format["Desguazado: %1 x Acero",_done] call OT_fnc_notifyMinor;
    };
...
if((count _filtered) isEqualTo 1) then {
    ...
    if(count _filtered > 1) then {
    	...
    	"Donde quieres meter la chatarra?" call OT_fnc_notifyBig;
    	_options call OT_fnc_playerDecision;
    }else{
        "No hay ningun contenedor para meter la chatarra" call OT_fnc_notifyMinor;
    };
};
