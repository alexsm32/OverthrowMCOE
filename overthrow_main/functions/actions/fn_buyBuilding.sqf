...
if(typename _b isEqualTo "ARRAY") then {
	...
	if !(_building call OT_fnc_hasOwner) then {
		...
		if(_owner isEqualTo getplayeruid player) then {
			...
			if(_home distance _building < 5) exitWith {"No puedes vender tu casa" call OT_fnc_notifyMinor;_err = true};
			_type = "sell";
			_handled = true;
		};
	};
...
if(_handled) then {
	...
	if(_type == "buy" && _money < _price) exitWith {"No puedes permitirtelo" call OT_fnc_notifyMinor};
	...
	if(_type isEqualTo "buy") then {
		...
		_owned pushback _id;
		[player,"Edificio Comprado",format["Comprado: %1 en %2 por $%3",getText(configFile >> "CfgVehicles" >> (typeof _building) >> "displayName"),(getpos _building) call OT_fnc_nearestTown,_price]] call BIS_fnc_createLogRecord;
		...
		if ((typeof _building) in OT_allRealEstate) then {
			...
			[player,"Edificio vendido",format["Vendido: %1 en %2 por $%3",getText(configFile >> "CfgVehicles" >> (typeof _building) >> "displayName"),(getpos _building) call OT_fnc_nearestTown,_sell]] call BIS_fnc_createLogRecord;
	...
	if !(_isfactory) then {
		"No hay edificios a la venta por aqui" call OT_fnc_notifyMinor;
	};
};
