...
_text = "<t align='center' size='0.7'>Las Cuentas (cada 6 hrs)</t><br/>";

if(isMultiplayer) then {
	if((getplayeruid player) in (server getVariable ["generals",[]])) then {
		_text = _text + format["<t align='left' size='0.65'>Caja de La Resistencia: $%1</t><br/>",[server getVariable ["money",0], 1, 0, true] call CBA_fnc_formatNumber];
	};
};

_text = _text + format["<t size='0.65'>Ingresos por alquileres: $%1</t><br/>",[_lease, 1, 0, true] call CBA_fnc_formatNumber];

if(isMultiplayer) then {
    _text = _text + format["<t size='0.65'>Ingresos por civiles: $%1 ($%2 per player)</t><br/>",[_taxtotal-_totax, 1, 0, true] call CBA_fnc_formatNumber,[_taxper-_totaxper, 1, 0, true] call CBA_fnc_formatNumber];
    if((getplayeruid player) in (server getVariable ["generals",[]])) then {
        _text = _text + format["<t size='0.65'>Impuestos (%1%2): $%3 ($%4 por jugador)</t><br/>",_tax,"%",[_totax, 1, 0, true] call CBA_fnc_formatNumber,[_totaxper, 1, 0, true] call CBA_fnc_formatNumber];
    };
}else{
    _text = _text + format["<t size='0.65'>Ingresos por Impuestos: $%1</t><br/>",[_taxtotal, 1, 0, true] call CBA_fnc_formatNumber];
};
_text = _text + format["<t size='0.65'>Ingresos por negocios: $%1 (6 hrs)</t><br/>",[_income, 1, 0, true] call CBA_fnc_formatNumber];

private _minus = "";
if(_wages > 0) then {_minus = "-"};
_text = _text + format["<t size='0.65'>Salarios: $%1%2 (6 hrs)</t><br/>",_minus,[_wages, 1, 0, true] call CBA_fnc_formatNumber];
if(isMultiplayer) then {
    _text = _text + format["<t size='0.8'>Balance (Resistancia): $%1</t><br/>",[_balance+_income, 1, 0, true] call CBA_fnc_formatNumber];
    _text = _text + format["<t size='0.8'>Balance (Tu): $%1</t><br/>",[_lease + (_taxper-_totaxper), 1, 0, true] call CBA_fnc_formatNumber];
}else{
