...
_text = _text + format["<t size='0.65'>Empleados: %1</t><br/>",_anum];
_text = _text + format["<t size='0.65'>Salarios: $%1 /hr</t><br/>",_wages];
...
if(count _data > 2) then {
    ...
    if(_input != "") then {
        _text = _text + format["<t size='0.65'>Necesita: %1 x %2 /hr</t><br/>",_innum, _input call OT_fnc_weaponGetName];
    };
    if(_output != "") then {
        _text = _text + format["<t size='0.65'>Produce: %1 x %2 /hr</t><br/>",_outnum, _output call OT_fnc_weaponGetName];
    }else{
        _sellprice = round(([OT_nation,_input,0] call OT_fnc_getSellPrice) * 1.2);
        _text = _text + format["<t size='0.65'>Ingresos: $%1 /hr</t><br/>",round(_innum * _sellprice)];
    };
}else{
    _text = _text + format["<t size='0.65'>Ingresos: $%1 /hr</t><br/>",round(_num * 200)];
};
_text = _text + format["<t size='0.65'>Siguiente ciclo: %1:00</t><br/>",_nexthr];
